...
import { ImageLightbox, LightboxImage } from "@/components/common/image-lightbox"
...
export function GalleryPageContent({
  ...
  const galleryImages = apiGalleries.length > 0 ? transformApiGalleriesToImages() : staticGalleryImages;
  ...
  const buildCategories = () => {
    ...
    if (apiCategories.length === 0) {
      ...
      return [
        allCategory,
        { id: "classrooms", name: "Classrooms", icon: Building, count: galleryImages.filter((i) => i.category === "classrooms").length },
        { id: "students", name: "Students", icon: Users, count: galleryImages.filter((i) => i.category === "students").length },
    ...
    <>
      ...
      <div className="text-center space-y-6">
        <div className="inline-flex items-center px-6 py-3 rounded-full bg-gradient-to-r from-purple-400/20 to-blue-500/20 border border-purple-400/30 backdrop-blur-sm">
          ...
          <span className="text-purple-400 font-medium">{isPreview ? "GALLERY" : "PHOTO GALLERY"}</span>
        </div>
        <h1 className={isPreview ? "text-4xl lg:text-6xl font-bold" : "text-4xl lg:text-6xl font-bold"}>
  ...
  {categoriesLoading ? (
    ...
    categories.map((category) => (
      ...
      {galleriesError && (
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
          <div className="text-red-500 font-medium">Error loading galleries: {galleriesError}</div>
          <Button onClick={() => {
            clearError();
            ...
            <div className="relative bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-all duration-500 hover:scale-105">
              ...
              <div className="p-4">
                ...
                <div className="flex mt-4 justify-between items-center text-xs text-gray-500">
                  <span><DateDisplay date={image.date}/></span>
                  <span className="flex items-center gap-1"> <MapPin className="h-4 w-4" /> {image.location}</span>
                </div>
              </div>
            </div>
          </div>
