      ...
      >
        <div className="container mx-auto px-4 py-8">
          <nav className="space-y-6">
            {NAVIGATION_ITEMS.map((item, index) => {
              ...
              return (
                <button
                  ...
                  onClick={() => handleNavigation(item.href, item.name)}
                  className={cn(
                    "block text-2xl font-medium transition-all duration-300 hover:translate-x-4 text-left w-full",
                    isActive ? "text-yellow-400" : "text-white/80 hover:text-white",
                  )}
                  style={{
            ...
            <Button
              onClick={() => handleNavigation("#contact", "Contact")}
              className="mt-8 bg-gradient-to-r from-yellow-400 to-yellow-600 text-black font-bold px-8 py-3 rounded-full w-full"
            >
              Get Started
            </Button>
          </nav>
