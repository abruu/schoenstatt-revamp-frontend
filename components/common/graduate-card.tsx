...
import { Calendar, GraduationCap, Trophy, Award, Medal, Star, Users } from "lucide-react"
...
const getGraduateIcon = (achievement: string, level: string) => {
  ...
  if (achievementLower?.includes('highest') || achievementLower?.includes('top')) return Trophy
  if (achievementLower?.includes('excellence') || achievementLower?.includes('outstanding')) return Award
  if (achievementLower?.includes('best') || achievementLower?.includes('perfect')) return Medal
...
export const GraduateCard = memo(({ graduate, onClick, className }: GraduateCardProps) => {
  const Icon = getGraduateIcon(graduate.achievement, graduate.language_certification_level.LabelShort)
        ...
        <CardContent className="p-0">
          ...
          <div className={`bg-gradient-to-r ${gradientClass} p-6 relative overflow-hidden`}>
            ...
            <div className="relative z-10 text-center">
              ...
              <h3 className="text-xl font-bold text-white mb-2">{graduate.StudenName}</h3>
