...
const FALLBACK_DATA = {
  ...
  testimonials: [
    {
      StudentName: "Maria K.",
      testimonialDescription: "Best German learning experience!",
    },
    {
      StudentName: "John D.",
      testimonialDescription: "Excellent teaching methodology",
    },
    {
      StudentName: "Sarah L.",
      testimonialDescription: "Achieved B2 level in 8 months",
    },
  ],
  headerimage: [
    {
...
export function HeroSection() {
  ...
  const { heroData, heroDataLoading, heroDataError, fetchHeroData } =
    useApiStore();
  ...
  useEffect(() => {
    ...
    const testimonialInterval = setInterval(() => {
      setCurrentTestimonial(
        (prev) => (prev + 1) % displayData.testimonials.length,
      );
    }, 4000);
  ...
  const prevImage = () => {
    ...
    setCurrentImageIndex(
      (prev) =>
        (prev - 1 + displayData.headerimage.length) %
        displayData.headerimage.length,
    );
    setTimeout(() => setIsTransitioning(false), 500);
  };
  ...
  const currentTestimonialData =
    displayData?.testimonials?.[currentTestimonial] ||
    displayData?.testimonials?.[0];
                ...
                <div className="relative bg-gradient-to-br from-white/15 to-white/5 backdrop-blur-2xl border border-white/30 rounded-[2rem] overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-500 group-hover:scale-[1.02]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-black/20">
                    <div
                      ...
                      style={{
                        width: `${((currentImageIndex + 1) / displayData.headerimage.length) * 100}%`,
                      }}
                    />
                  </div>
                </div>
