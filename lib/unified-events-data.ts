...
export const mapApiEventsToUnifiedFormat = (apiEvents: any[]): UnifiedEvent[] => {
  ...
  return apiEvents.map(event => {
    ...
    const galleryItems = event.GalleryItems?.map((item: any) => ({
      id: item.id,
      src: item.src?.url || '',
      alt: item.alt || '',
      title: item.title || '',
    ...
    return {
      ...
      excerpt: event.excerpt || event.description?.substring(0, 100) + '...' || '',
...
export const getUnifiedEventsData = (): UnifiedEvent[] => {
  ...
  const eventsData = events.length > 0 ? mapApiEventsToUnifiedFormat(events) : staticEventsData;
